 ...
 * 	'len' if the maximum file size is exceeded, or if an error happened
 *       after some bytes had been written), or -1 in case of error
 */
 ...
 * 	(can be lower than 'len' if the file size was reached, or if an error
 *      happened after some bytes had been read), or -1 in case of
 * error
 */
